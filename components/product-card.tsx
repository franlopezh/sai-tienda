...
function getBadge(p: Producto): { label: string; tone: string } | null {
  // Agotado tiene prioridad sobre cualquier otro badge.
  if (p.activo === false) {
    return { label: "Agotado", tone: "bg-zinc-700 text-white" };
  }
  ...
  return (
    <Link href={`/producto/${producto.slug}`} className="group block h-full">
      <Card className="flex h-full flex-col overflow-hidden transition duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:border-blue-300 animate-in fade-in-0 slide-in-from-bottom-2 fill-mode-both">
        <div className="relative h-44 w-full shrink-0 overflow-hidden flex items-center justify-center text-muted-foreground/40 p-4 md:h-52">
          {badge && (
            <span
              className={`absolute top-3 left-3 z-20 rounded-full px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide shadow-sm ${badge.tone}`}
            >
              {badge.label}
            </span>
          )}
          {producto.imagen_url ? (
            <>
              {/* Ambient: misma imagen escalada al doble y blur extremo →
                  funciona como un campo de color que extiende el tono del
                  borde de la foto. Casi no se ve la imagen, solo el color. */}
              ...
              <img
                ...
                loading="lazy"
                className={cn(
                  "relative z-10 max-h-full max-w-full object-contain transition duration-300 group-hover:scale-[1.05]",
